...
export class DemoComponent extends React.Component<{ id: string }, undefined>{
    ...
    constructor(props) {
        ...
        if (this.demo) {
            ...
            // this.htmlTemplate = `${this.html}<style>${this.css}</style><script>${this.script}</script>`
            this.codeEditorHeight = this.calcHeight();
        }

    }
    public getDemoTemplate():string{
        return  `<style>${this.css}</style>${this.html}<script>${this.script}</script>`;
    }
